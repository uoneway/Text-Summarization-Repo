...
| 2017<br /><br />Model                 | [**SummaRuNNer**: A Recurrent Neural Network based Sequence Model for Extractive Summarization of Documents](https://arxiv.org/abs/1611.04230)<br />R. Nallapati, F. Zhai and B. Zhou<br /><br />![img](images/summarization.005.png) <br />문장별로 BiGRU에 word 단위로 입력하여 encoding한 후, sentence 단위로 한 번 더 BiGRU에 입력하여 문장 임베딩(hidden states)을 생성합니다. 개별 문장 임베딩과 이들을 wighted sum한 doc를  logistic classifier에 입력해 해당 문장이 요약에 포함시킬지 여부를 판단합니다. | ext,<br />RNN<br />(hierarchical BiGRU)              |
| 2017<br /><br />Model,<br />Technique | [Get to the point: Summarization with pointergenerator networks](https://arxiv.org/abs/1704.04368) ([Code](https://github.com/abisee/pointer-generator))<br />A. See, P. J. Liu, and C. D. Manning / ACL 2017<br /><br />![img](images/summarization.011.png)<br /> Generator를 통해 vocabulary로부터 단어를 생성할지(abs), pointer를 통해 원문을 copy할지(ext)를 학습된 생성 확률(Pgen)에 따라 결정합니다. 또한 특정 단어가 반복해서 생성되는 문제를 해결하고자, 현재까지 사용된 단어별 누적 attention distribution값(coverage vector)을 repetition penalty term으로 포함하는 coverage loss를 사용합니다. <br /><br />- [Review] [김형석(KoreaUniv DSBA)](https://brunch.co.kr/@kakao-it/139) | ext/abs,<br />Pointer-Generator, <br />Coverage loss |
